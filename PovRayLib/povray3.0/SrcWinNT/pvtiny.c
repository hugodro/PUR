...
*                pvtiny.c
*
*  This module implements Windows 'tiny caption' non-client drawing routines.
*
...
#define ISNEWWINDOW(hwnd)     (TestWinStyle (hwnd, IBS_VERTCAPTION) && \
                               TestWinStyle (hwnd, IBS_HORZCAPTION))

#define SETCAPTIONSIZE(h,i)   SetProp(h,"ibSize", (HANDLE) i)
#define GETCAPTIONSIZE(h)     GetProp(h,"ibSize")
#define FREECAPTIONSIZE(h)    RemoveProp(h,"ibSize")

#define SETMENUWASUPFLAG(h,i) SetProp(h,"ibFlag", (HANDLE) i)
#define GETMENUWASUPFLAG(h)   GetProp(h,"ibFlag")
...
BOOL NEAR PASCAL ibDrawCaption (HDC hDC, HWND hWnd, LPRECT lprc,
...
UINT WINAPI ibGetCaptionSize (HWND hWnd )
{
    return (UINT) GETCAPTIONSIZE (hWnd) + 1 ;
} // ibGetCaptionSize()
...
UINT WINAPI ibSetCaptionSize (HWND hWnd, UINT nSize)
{
    ...
    nSize-- ;
    ui = (UINT) SETCAPTIONSIZE (hWnd, nSize) ;
    ...
    // CJC on my system, when this was called, it seemed to cause the
    // window position to move by the amount of the caption. since I
    // only call it when my window is hidden, it's not important.
...
LRESULT WINAPI ibDefWindowProc (HWND hWnd, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uiMsg)
    {
      ...
      case WM_COMMAND:
           // The menu that is popped up for the system menu with
           ...
           if (wParam >= 0xF000)
           {
               ...
               //      TPM() returns to our code within DoMenu() below.
      ...
      case WM_GETMINMAXINFO:
           nCapSize = (UINT) GETCAPTIONSIZE (hWnd) ;
      ...
      case WM_NCCREATE:
           ...
           // was specified the caller made a mistake. Things get really
           // ugly if DefWindowProc sees WS_DLGFRAME, so we strip
           // the WS_DLGFRAME part off!
           //
           if ((style & IBS_VERTCAPTION || style & IBS_HORZCAPTION) && style & WS_DLGFRAME)
           {
             style &= (DWORD) ~WS_DLGFRAME ;
             SetWindowLong (hWnd, GWL_STYLE, style) ;
           }
           return (DefWindowProc (hWnd, uiMsg, wParam, lParam)) ;
      ...
      case WM_NCCALCSIZE:
           ...
           // then we trap this message and calculate the client area rect,
           // which is the client area rect calculated by DefWindowProc()
           // plus the width/height of the mini-caption bar
           //
           lRet = DefWindowProc (hWnd, uiMsg, wParam, lParam) ;
...
//           nCapSize = (UINT) GETCAPTIONSIZE (hWnd) ;
      ...
      case WM_NCLBUTTONDOWN:
           ...
           // If you wanted to change the behavior of the min/max buttons to
...
//           if (uiMsg == WM_NCPAINT)
//               fActive = (hWnd == GetForegroundWindow ()) ;
//           else
//               fActive = wParam ;

             fActive = (hWnd == GetForegroundWindow ()) ;
             ibDrawCaption (hDC, hWnd, &rcCap,
...
VOID WINAPI ibAdjustWindowRect (LPRECT lprc, UINT flags, UINT nCapSize)
{
  ...
  //
  if ((flags & WS_THICKFRAME) != 0)
  {
    cx = GetSystemMetrics (SM_CXFRAME) ;
    cy = GetSystemMetrics (SM_CYFRAME) ;
  }
  else
    if ((flags & DS_MODALFRAME) != 0)
    {
  ...
  if ((flags & IBS_VERTCAPTION) != 0)
    lprc->left -= nCapSize ;
  else
    if ((flags & IBS_HORZCAPTION) != 0)
      lprc->top -= nCapSize ;

} // ibAdjustWindowRect() 

//  ibAdjustWindowRectHWND  (HWND hWnd, LPRECT lprc)
//
//    Does the same thing as the USER function AdjustWindowRect(),
//    but knows about itsybitsy windows.  AdjustWindowRect() is
//    bogus for stuff like this.
...
{             
  ...
  DrawButton (NULL, hWnd, uiHT == HTMINBUTTON, TRUE) ;
  SetCapture (hWnd) ;
  ...
  while (TRUE)
  {
    if (PeekMessage ((LPMSG) &msg, NULL, WM_MOUSEFIRST, WM_MOUSELAST, PM_REMOVE))
    {
      switch (msg.message)
      {
        case WM_LBUTTONUP:
             if (fDepressed)
               DrawButton (NULL, hWnd, uiHT == HTMINBUTTON, FALSE) ;
             ReleaseCapture () ;
  ...
  if (fWin31)
  {
    ...
    if ((lpsz = GlobalAllocPtr (GHND, ui + 2)) != NULL)
    {
      ...
      if (fVert)
      {
        ...
        colour = SetTextColor (hDC, RGBWHITE) ;
        if (tm.tmPitchAndFamily & TMPF_VECTOR || tm.tmPitchAndFamily & TMPF_TRUETYPE)
        {
          ExtTextOut (hDC,
                      cy,
                      min (cx, rcCap.bottom),
...
void NEAR PASCAL SetupSystemMenu (HWND hWnd, HMENU hMenu)
{
  ...
  if (TestWinStyle (hWnd, WS_THICKFRAME) && !(IsIconic (hWnd) || IsZoomed (hWnd)))
    wSize = MF_ENABLED ;
  
  if (!IsZoomed (hWnd) && !IsIconic (hWnd) && TestWinStyle (hWnd, WS_CAPTION))
    wMove = MF_ENABLED ;
...
//
//    calculates the rectangle of the mini-caption in screen coords.
//
BOOL NEAR PASCAL GetCaptionRect (HWND hWnd, LPRECT lprc)
{
  UINT     nCapSize ;
...
BOOL NEAR PASCAL GetButtonRect (HWND hWnd, UINT nPos, LPRECT lprc)
{
  ...
  nSize = (UINT) GETCAPTIONSIZE (hWnd) ;
...
VOID NEAR PASCAL DrawButton (HDC hDC, HWND hWnd, BOOL fMin, BOOL fDepressed)
{
  ...
  // pixels.  That's what this THRESHOLD stuff does.
  //
  #define THRESHOLD   20
  ...
  if (hDC)
  {
    ...
    //
    n = ((GetSystemMetrics (SM_CYCAPTION)) / 2) - (rcButton.right - rcButton.left) ;
    if (n < 1)
