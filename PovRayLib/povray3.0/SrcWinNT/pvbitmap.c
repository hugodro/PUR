...
// HPALETTE hOldPal;           // handle to a palette
   ...
   // for the splash screen bitmap in POV-Ray for Windows, we manually set the
   // biClrImportant field (in the file itself) to the actual number of colours
   // used, which in this case (at the time of writing) was 174. this leaves room
   // in the system palette for some other programs and ought to reduce the
   // kaledioscope effect.
   create_palette (((BITMAPINFO *) lpDIBHdr)->bmiColors, ((BITMAPINFO *) lpDIBHdr)->bmiHeader.biClrImportant) ;
// hOldPal = SelectPalette(hDC, hPalApp, FALSE);
