/**************************************************
* File: povray_lib.c.
* Desc: Implementation of the main control routines for PovRay, when used
*	as a dynamic library tool.
* Module: AkraLog : VrmlSup.
* Rev: 26 avril 1998 : REV 0 : Hugo DesRosiers : Creation.
**************************************************/
...
#include "quadrics.h"
#include "pgm.h"
/* MOD-980920 [HD]: Took this file out, not needed.
#include "png_pov.h"
*/
#include "ppm.h"
#include "prism.h"
...
static void setup_output_file_name PARAMS((char *));
...
/* Things added for dynamic lib. */
static void povLib_Init_Text_Streams(void);
static void pvLib_pre_init_povray(void);
void EntryPointPovRayLib PARAMS((void));
void CreateDefaultsPovRayLib PARAMS((void));
void FrameRenderPovRayLib PARAMS((char *aName));
void ExitPointPovRayLib PARAMS((void));
void setOptionsPovRayLib PARAMS((int optID, void *optValue));


void EntryPointPovRayLib(void)
{
    register int i;
    int Diff_Frame;
    DBL Diff_Clock;
    ...
    pvLib_pre_init_povray();

  /* Startup povray. */
#if defined(USE_STARTUP)
    Stage = STAGE_STARTUP;
    STARTUP_POVRAY
#endif
    ...
    Stage = STAGE_INI_FILE;	  /* Read parameters from POVRAY.INI */
    PROCESS_POVRAY_INI

    Open_Text_Streams();    /* Redirect text streams [SCD 2/95] */
    fix_up_animation_values();    /* Make sure clock is okay, validate animation parameters */
    fix_up_rendering_window();    /* Fix-up rendering window values if necessary. */
    Initialize_Noise();    /* Set up noise-tables. */
...
void CreateDefaultsPovRayLib(void)
{
    Default_Camera= Create_Camera();
    Default_Texture= Create_Texture();
    Default_Texture->Pigment= Create_Pigment();
...
void FrameRenderPovRayLib(char *aName)
{
    SHELLRET Frame_Result;

    strcpy(opts.Scene_Name, aName);

    set_output_file_handle();    /* Set output file handle for options screen. */
...
static void FrameRender(void)
{
    ...
    Build_Bounding_Slabs(&Root_Object);    /* Always call this to print number of objects. */
    Build_Vista_Buffer();	/* Create the vista buffer. */
    Build_Light_Buffers();    /* Create the light buffers. */
    Init_Blob_Queue();    /* Create blob queue. */

    variable_store(STORE);    /* Save variable values. */
...
static void set_output_file_handle()
{
  ...
  char temp[FILE_NAME_LENGTH];
	...
	case 's' :
	case 'S' :
	    Output_File_Handle= GET_SYS_FILE_HANDLE();
	    def_ext=SYS_DEF_EXT;
	    break;
	case 't' :
	case 'T' :
	...
	case 'p' :
	case 'P' :
	    Output_File_Handle= Get_PPM_File_Handle();
	    def_ext=".ppm";
	    break;
	case 'n' :
	case 'N' :
	    Output_File_Handle= Get_Png_File_Handle();
	    def_ext=".png";
	    break;
	case 'd' :
	case 'D' :
	...
	default :
	    Error ("Unrecognized output file format %c.\n", opts.OutputFormat);
	    break;
    }
...
*
******************************************************************************/
static void setup_output_file_name(char *aName)
{
    ...
    if ((opts.FrameSeq.FrameType != FT_MULTIPLE_FRAME) || (opts.Options & TO_STDOUT)) {
	strcpy(opts.Output_Numbered_Name, opts.Output_File_Name);
    }
    else {
	    /*
...
*/
	strcat(opts.Output_Numbered_Name, aName);
...
* FUNCTION
*
*   void pvLib_pre_init_povray(void)
*
* INPUT -- none
*
* OUTPUT
...
void povLib_Init_Text_Streams(void)
{
    int i;
	...
	Stream_Info[i].do_console = FALSE;
    }
}
...
void setOptionsPovRayLib(int optID, void *optValue)
{
    switch(optID) {
	case 1:	/* Output file name. */
	    strncpy(opts.Output_File_Name, (char *)optValue, FILE_NAME_LENGTH);
	    break;
	case 2:	/* Output image format. */
	    opts.OutputFormat = (char)tolower((char)(int)optValue);
	    break;
	case 3:	/* Width x Height. */
	    Frame.Screen_Width= ((int *)optValue)[0];
	    Frame.Screen_Height= ((int *)optValue)[1];
	    break;
    }
}
